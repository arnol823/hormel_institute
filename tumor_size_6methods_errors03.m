% edited from tumor_size_6methods_errors02.m
% adds interpolation to use smaller h for finite difference method
...
for pid = 1:length(patientIDs)
    ...
    finer_t_data = linspace(min(t_data), max(t_data), 10000 * length(t_data));
    finer_V = interp1(t_data, V, finer_t_data, 'pchip');
    ...
    tic;
    num_points = length(finer_V) - 1;
    V_matrixA_fwd = zeros(num_points, 2);
    V_matrixB_fwd = zeros(num_points, 1);

    for i = 1:num_points
        V_matrixA_fwd(i, :) = [h(i) * finer_V(i)^(2/3), -h(i) * finer_V(i)]; 
        V_matrixB_fwd(i) = finer_V(i+1) - finer_V(i); 
    end
    ...
    [t_ode_fwd, V_ode_fwd] = ode45(dVdt_fwd, finer_t_data, finer_V(1));
    ...
    for i = 2:num_points+1
        V_matrixA_bwd(i-1, :) = [h(i-1) * finer_V(i-1)^(2/3), -h(i-1) * finer_V(i-1)]; 
        V_matrixB_bwd(i-1) = finer_V(i) - finer_V(i-1); 
    end
    ...
    [t_ode_bwd, V_ode_bwd] = ode45(dVdt_bwd, finer_t_data, finer_V(1));
    ...
    tic;
    num_points_central = length(finer_V) - 2;
    V_matrixA_central = zeros(num_points_central, 2);
    V_matrixB_central = zeros(num_points_central, 1);

    for i = 2:num_points_central+1
        V_matrixA_central(i-1, :) = [h(i) * (finer_V(i+1)^(2/3) + finer_V(i-1)^(2/3)) / 2, -h(i) * (finer_V(i+1) + finer_V(i-1)) / 2]; 
        V_matrixB_central(i-1) = finer_V(i+1) - finer_V(i-1); 
    end
    ...
    [t_ode_central, V_ode_central] = ode45(dVdt_central, finer_t_data, finer_V(1));
